...
Şekle baktığımızda gerçekten Doğu Anadolu fayı ekseninde bir hareketlilik göze çarpıyor.

Bu hareketi daha iyi göstermek için bir animasyon daha oluşturalım (@fig-quake-4).
...
data %>% 
  ...
  filter(Latitude < 40) %>%
    ggplot()+
      ...
      geom_point(aes(y = Latitude, x = Longitude, fill = Magnitude, size = Magnitude, alpha=alpha), 
       shape=20, color = "NA") +
      transition_manual(Date, cumulative = F) +
      scale_fill_gradientn(colours = c("#15607a", "#7495a6", "#c7cdd1", "#cc734d", "#a63716")) + 
      xlim(c(25,45)) + ylim(c(35,42.5)) +
      theme(legend.position = "node")
```
